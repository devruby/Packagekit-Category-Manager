...
!function (a, b) {
    ...
    function L(a, b, c) {
        if (b.enablePagination) {
            var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>', e = '<li><a href="#{0}" role="menuitem" class="btn-nhap">{1}</a></li>', f = "";
            b.forceMoveForward || b.enableCancelButton && (f += e.format("cancel", b.labels.cancel)), (f += e.format("previous", b.labels.previous)), b.enableFinishButton && (f += e.format("finish", b.labels.finish)), f += e.format("next", b.labels.next), a.append(d.format(b.actionContainerTag, b.clearFixCssClass, b.labels.pagination, f)), D(a, b, c), A(a, b, c)
        }
    }
